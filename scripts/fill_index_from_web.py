...
- 公开募资(倍) (retail fundraising amount) -> HKD, formatted as 万/亿港元
...
def main() -> int:
    ...
    need_cols = {
        ...
        "股票名称": idx.get("股票名称"),
        "中签率": idx.get("中签率"),
        "公开募资(倍)": idx.get("公开募资(倍)"),
        "配售超购倍数": idx.get("配售超购倍数"),
        "公开发售超购倍数": idx.get("公开发售超购倍数"),
    }
    if any(need_cols[k] is None for k in ("股票名称", "中签率", "公开募资(倍)", "配售超购倍数", "公开发售超购倍数")):
    ...
    for tr in soup.select("table tbody tr"):
        ...
        miss = {
            ...
            "retail": is_missing_text(tds[need_cols["公开募资(倍)"]].get_text(strip=True)),
        ...
        if miss["retail"] and found.retail_hkd is not None:
            row_updates["公开募资(倍)"] = fmt_hkd_amount(found.retail_hkd)
        ...
        if row_updates:
            ...
            if not args.dry_run:
                ...
                if "公开募资(倍)" in row_updates:
                    tds[need_cols["公开募资(倍)"]].string = row_updates["公开募资(倍)"]
