...
DOCS = ROOT / "docs"
REQUIRED_COLS = ["代码", "股票名称", "中签率", "配售超购倍数", "公开发售超购倍数"]
...
def get_index_map(soup: BeautifulSoup) -> dict[str, int]:
    headers = [norm_header(th.get_text(" ", strip=True)) for th in soup.select("table thead th")]
    idx = {h: i for i, h in enumerate(headers)}
    missing = [col for col in REQUIRED_COLS if col not in idx]
    if missing:
...
def count_missing_rows(soup: BeautifulSoup, idx: dict[str, int]) -> int:
    n = 0
    hit_i = idx["中签率"]
    place_i = idx["配售超购倍数"]
    public_i = idx["公开发售超购倍数"]
    for tr in soup.select("table tbody tr"):
        tds = tr.find_all("td")
        if len(tds) <= max(hit_i, place_i, public_i):
            continue
        if has_missing(tds[hit_i]) or has_missing(tds[place_i]) or has_missing(tds[public_i]):
            n += 1
    return n
...
def main() -> None:
    ...
    missing_before = count_missing_rows(soup, idx)
    scanned = 0
    touched = 0
    ...
    for tr in rows:
        tds = tr.find_all("td")
        if len(tds) <= max(code_i, name_i, hit_i, place_i, public_i):
            continue
        code = tds[code_i].get_text(strip=True).zfill(5)
        name = tds[name_i].get_text(strip=True)
        td_hit, td_place, td_public = tds[hit_i], tds[place_i], tds[public_i]
    ...
    missing_after = count_missing_rows(soup, idx)
